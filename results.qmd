...
### A Chloropleth Map of Average CO<sub>2</sub> Emissions Over 3 decades
```{r}
#| fig-height: 7
#| fig-width: 15
#| warning: false
...
# country_co2$mean_emissions[is.na(country_co2$mean_emissions)] <- 0
...
ggplot(emissions_covid, aes(x = Year, y = CO2_Emissions)) +
  geom_boxplot() +
  labs(
    title = "CO2 Emissions before and during the 2020 lockdown",
    x = "Year",
    y = "CO"[2] ~ "(10"^ 6~ "kg)") +
  theme_minimal()
```
...
ggplot(missing_GDP, aes(x = avg_GDP, y = Country.Name)) +
  geom_bar(stat = "identity", fill = "skyblue") +
  labs(title = "Top 10 Nations with Limited CO"[2] ~ "Emission Reporting and their GDP", x = "GDP (USD Billion)", y = "Country Code") +
